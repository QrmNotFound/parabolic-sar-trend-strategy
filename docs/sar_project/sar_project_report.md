...
本项目重做原本科 SAR 策略报告，重点展示数据获取、回测偏差修正、交易约束建模、样本内参数选择、历史验证和结果解释能力。本报告使用 tinyshare/Tushare 接口下载的历史行情与指数成分数据生成。
...
历史验证累计收益对比：策略 -9.92%，沪深300 -12.11%，动态前50等权毛收益组合 -8.19%。本版策略在扣除交易成本后相对沪深300取得正累计超额，但绝对收益仍为负，不能被表述为稳定盈利策略。该结果属于课程级历史回测，不等同于可实盘收益承诺。
